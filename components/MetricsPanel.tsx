...
export interface Ex{id:string;name:string;reps:number;desc:string}
export const EXERCISES:Ex[]=[
 {id:'finger_flexion',name:'Finger Flexion',reps:10,desc:'Grip & release'},{id:'finger_extension',name:'Finger Extension',reps:10,desc:'Full open spread'},
 {id:'individual',name:'Individual Isolation',reps:8,desc:'Finger-by-finger'},{id:'power_grip',name:'Power Grip',reps:12,desc:'Max force hold 3s'},
 {id:'pinch',name:'Pinch Grip',reps:10,desc:'Thumb + index'},{id:'wave',name:'Wrist Flexion',reps:10,desc:'Wrist up/down'}]
...
function Select({sel,on,disabled}:any){const [o,setO]=useState(false),r=useRef<HTMLDivElement>(null)
 ...
 {o&&!disabled&&<div className="absolute left-0 top-full mt-1 min-w-full w-[220px] bg-[#161616] border border-[#0F6E5E] rounded-lg z-50 overflow-hidden">{EXERCISES.map(e=>
  ...
  <span><span className="block text-[10pt]" style={{color:e.id===sel.id?'#0F6E5E':'#f0f0f0'}}>{e.name}</span><span className="block text-[8pt] text-[#555]">{e.desc}</span></span></button>)}</div>}</div>}
